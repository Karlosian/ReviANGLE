...
// glBufferData / glBufferSubData — allow ANGLE to handle validation natively
...
typedef void* (WINAPI *PFN_MBR)(GLenum, GLintptr, GLsizeiptr, GLbitfield);
extern "C" __declspec(dllexport) void* WINAPI gl_glMapBufferRange(GLenum target, GLintptr offset, GLsizeiptr length, GLbitfield access) {
    static PFN_MBR p = nullptr;
    if (!p) p = (PFN_MBR)glproxy::resolve("glMapBufferRange");
    void* result = p ? p(target, offset, length, access) : nullptr;
    if (!result) {
        typedef GLenum (WINAPI *PFN_GE)();
        static PFN_GE pGetError = nullptr;
        if (!pGetError) pGetError = (PFN_GE)glproxy::resolve("glGetError");
        GLenum err = pGetError ? pGetError() : 0;
        angle::log("glMapBufferRange FAILED: target=0x%x offset=%lld length=%lld access=0x%x -> err=0x%x",
                   target, (long long)offset, (long long)length, access, err);
    }
    return result;
}
typedef void* (WINAPI *PFN_MAPBUF)(GLenum, GLenum);
extern "C" __declspec(dllexport) void* WINAPI gl_glMapBuffer(GLenum target, GLenum access) {
    static PFN_MAPBUF p = nullptr;
    if (!p) p = (PFN_MAPBUF)glproxy::resolve("glMapBuffer");
    if (!p) p = (PFN_MAPBUF)glproxy::resolve("glMapBufferOES");
    void* result = p ? p(target, access) : nullptr;

    if (!result) {
        typedef void (WINAPI *PFN_GBPIV)(GLenum, GLenum, GLint*);
        static PFN_GBPIV pGetParam = nullptr;
        static PFN_MBR pMapRange = nullptr;
        if (!pGetParam) pGetParam = (PFN_GBPIV)glproxy::resolve("glGetBufferParameteriv");
        if (!pMapRange) pMapRange = (PFN_MBR)glproxy::resolve("glMapBufferRange");

        if (pGetParam && pMapRange) {
            GLint size = 0;
            pGetParam(target, 0x8764 /*GL_BUFFER_SIZE*/, &size);
            if (size > 0) {
                GLbitfield accessFlags = 0;
                if (access == 0x88B8 /*GL_READ_ONLY*/)       accessFlags = 0x0001 /*GL_MAP_READ_BIT*/;
                else if (access == 0x88B9 /*GL_WRITE_ONLY*/)  accessFlags = 0x0002 /*GL_MAP_WRITE_BIT*/;
                else if (access == 0x88BA /*GL_READ_WRITE*/)  accessFlags = 0x0001 | 0x0002;
                result = pMapRange(target, 0, size, accessFlags);
            }
        }
    }
    ...
    if (!result) {
        ...
        if (!pGetError) pGetError = (PFN_GE)glproxy::resolve("glGetError");
        GLenum err = pGetError ? pGetError() : 0;
        angle::log("glMapBuffer FAILED: target=0x%x access=0x%x -> err=0x%x",
                   target, access, err);
    }
    return result;
}
...
extern "C" __declspec(dllexport) void WINAPI gl_glShaderSource(GLuint shader, GLsizei count, const GLchar* const* string, const GLint* length) {
    ...
    const bool needsDerivatives =
        isFragment &&
        (src.find("fwidth") != std::string::npos ||
         src.find("dFdx") != std::string::npos ||
         src.find("dFdy") != std::string::npos ||
         src.find("GL_OES_standard_derivatives") != std::string::npos);
    ...
    angle::log("ShaderSource patched shader=%u type=%s:\n%s", shader, isFragment ? "FRAG" : "VERT", patched.c_str());
}
...
extern "C" __declspec(dllexport) void WINAPI gl_glCompileShader(GLuint shader) {
    ...
    if (pgsv) {
        ...
        if (cs == 0) {
            ...
            angle::log("CompileShader FAIL shader=%u: %s", shader, buf);
        } else {
            angle::log("CompileShader OK shader=%u", shader);
        }
    }
}
...
extern "C" __declspec(dllexport) void WINAPI gl_glLinkProgram(GLuint program) {
    ...
    angle::log("glLinkProgram: prog=%u link_status=%d", program, ls);
    if (ls == 0 && pgpiv) {
...
extern "C" __declspec(dllexport) void WINAPI gl_glBindFramebuffer(GLenum target, GLuint framebuffer) {
    ...
    static int n = 0;
    if (n < 30) angle::log("glBindFramebuffer #%d: target=0x%X fb=%u", n, target, framebuffer);
    n++;
